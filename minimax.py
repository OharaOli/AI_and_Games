...
class AI:
	...
	def get_move_score(self, move):
		...
		while next_player == self.player and board_copy.check_move(self.player, move):
			...
			value = max(value, self.alphabeta(board_copy, -98, 98, next_player, self.lookahead))
	...
	def move_parallel(self, board):
		...
		try:
			...
			for i in range(0, 7): # ignore first move, already chosen
		...
		while (i < 7 and not cut):
			...
			if board_copy.check_move(self.player, i):
				...
				if (alpha > beta):
					cut = True
			i+=1
		print("Searched " + str(self.search_count) + " possibilities")
		return move
