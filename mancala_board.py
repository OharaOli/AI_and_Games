...
class Board:
	def __init__(self, other=None):
		if other: #make copy
			self.board = [7,7,7,7,7,7,7,7,7,7,7,7,7,7]
	...
	def get_pieces(self, player):
		...
		pieces = 0
		for i in range(0, 7):
			pieces += self.board[player*7+i]
		return pieces


	def check_move(self, player, move):
		return ( move >= 0 and move < 7) and  (0 != self.board[player*7+move])

	def has_move(self, player):
		# if the player has any moves
		i = 0
		while i < 7 and  self.board[(player)*7+i] == 0:
			i+=1
	...
	def game_over(self):
		...
		return (self.bowl[0] + self.bowl[1]) == 98


	# def __repr__(self):
	# 	layout = '--------------'+ str(self.move_num) + '---------------\n'
	# 	layout += 'P2:' + str(self.bowl[1]) + '      7 <-- 1   |\n       |'
	# 	# show in reverse for player 1
	# 	for p in reversed(self.board[6:14]):
	# 		layout += str(p) + ' '
	# 	layout += '|                    \n\n       |'
	# 	for p in self.board[0:7]:
	# 		layout += str(p) + ' '
	# 	layout += '|\n       |  1 -->       P1: ' + str(self.bowl[0]) + '\n--------------------------------'
	# 	return layout
